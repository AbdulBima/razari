...
const admissionSchema = z.object({
  ...
  ageGroup: z.enum(["Adult", "Child"], { message: "Age group is required" }),
...
export default function AddAdmissionRec() {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const onSubmit = async (data: AdmissionFormData) => {
    ...
    try {
      const response = await staffApi.post("/admissions/create", payload);
  ...
  return (
    <div className="poppins-regular w-full md:max-w-lg mx-auto shadow-lg mt-6 md:mt-10 px-6 md:px-6 py-8 bg-white rounded-3xl transition duration-300 ease-in-out">
      <h2 className="text-lg font-semibold text-gray-600 mb-6 text-center">Admission Form</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <select {...register("ageGroup")} className="mt-1 block w-full p-3 bg-gray-100 text-gray-900 rounded-xl focus:ring-2 focus:ring-[#356966] outline-none transition duration-200">
        ...
        <div>
          ...
          <input type="text" {...register("reason")} className="mt-1 block w-full p-3 bg-gray-100 text-gray-900 rounded-xl focus:ring-2 focus:ring-[#356966] outline-none transition duration-200" placeholder="Enter reason" />
